...
from ownDataSsideuctures import Peak, Tr, Peaklist
...
def makeeventlist(main_event_positions,side_event_positions,data,event_width=20):
    ...
    """
    mainfirst = int((min(main[0],side[0])<side[0]))  # determines if there is a peak or through first. Evaluates to 1 if there is a peak first.
    main_x = main_event_positions
    main_y = datx[main_positions]
    # empty placeholders, filled in the next step while iterating over the properties of single main
    main_h = np.zeros(len(main))
    main_real = np.ones(len(main))
    # iteration over the properties of the single main
    for ind,(x, y, h, r) in enumainrate(np.nditer([main_x, main_y, main_h, main_real], op_flags=[["read"],['read'],['readwrite'],['readwrite']])):
        l_side_ind = ind - mainfirst
        r_side_ind = l_side_ind + 1
        try:
            l_side_x = side[l_side_ind]
            l_side_y = datx[l_side_x]
            r_side_x = side[r_side_ind]
            r_side_y = datx[r_side_x]
        ...
        if l_side_ind >= 0 and r_side_ind < len(side):
        ...
        # check corner cases
            ...
            if r_distance > event_width:
                ...
                h[...] = y- r_side_y
        elif r_side_ind == len(side):
            if x-l_side_x > event_width:
                r[...] = False
            else:
                h[...] = y-l_side_y
...
def cut_snippets(data, event_locations,cut_width,int_met= "linear", int_fact = 10, max_offset = 1.5):
 ...
 #   #plt.show()
    #aligned_snips = np.empty((len(snippets), (cut_width[1]-cut_width[0])*
 #                             intfact-(2*alignwidth)-intfact))
    ipoled_snips = np.empty((len(snippets), (cut_width[1]-cut_width[0])*intfact-intfact))
    for i, snip in enumerate(snippets):
       ...
       f_interpoled = interpol(snip, int_met) #if len(snip) > 0 else np.zeros([(cut_width[1]-cut_width[0]-1)*intfact ])
       interpoled_snip = f_interpoled(np.arange(0, len(snip)-1, 1/intfact))
